var addressModel = {
    ...
    initialize: function (db, tran) {
        ...
        logSqlCommand(sql1);
        tran.executeSql(sql1, [], function (tx) { }, function (tx, dberr) { });
    ...
    insertDistricts: function (provinceId, districts, callback) {
        db.transaction(function (tx) {
            for (var i = 0; i < districts.length; i++) {
                ...
                var sql = ('INSERT INTO [District] VALUES (' + 
                    '"' + district.id + '", ' +
                    '"' + district.name + '", ' +
                    '"' + provinceId + '")');
                logSqlCommand(sql);
                ...
                for (var j = 0; j < district.wards.length; j++) {
                    ...
                    var sql1 = ('INSERT INTO [Ward] VALUES (' +
                        '"' + ward.id + '", ' +
                        '"' + ward.name + '", ' +
                        '"' + district.id + '")');
                    logSqlCommand(sql1);
    ...
    isDistrictsDownloaded: function (provinceId, callback) {
        db.transaction(function (tx) {
            ...
            tx.executeSql(sql, [], function (tx, dbres) { callback(dbres.rows.length > 0); }, function (dberr) { callback(false); });
        });
    },
