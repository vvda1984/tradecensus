...
function initializeOutlet(outlet) {
  ...
  if (outlet.Distance == null) outlet.Distance = 0;
  ...
  outlet.IsNew =
    ...
    outlet.AuditStatus == StatusAuditDeny ||
  ...
  outlet.canPost =
    !user.hasAuditRole &&
    ((outlet.AuditStatus == StatusNew && outlet.PersonID == user.id) || (outlet.AuditStatus == StatusEdit && outlet.AmendBy == user.id));
  outlet.canApprove = outlet.AuditStatus == StatusAuditorNew && outlet.PersonID == userID;
  outlet.canRevise =
    !user.hasAuditRole &&
    ((outlet.AuditStatus == StatusPost && outlet.PersonID == userID) || (outlet.AuditStatus == StatusExitingPost && outlet.AmendBy == userID));
  ...
  if (outlet.AuditStatus == StatusAuditAccept || outlet.AuditStatus == StatusExitingAccept || outlet.AuditStatus == StatusAuditorAccept) {
    ...
    (outlet.auditResult = "Approved"), (outlet.viewAuditStatus = true);
  } else if (outlet.AuditStatus == StatusAuditDeny || outlet.AuditStatus == StatusExitingDeny) {
    outlet.IsAudited = false;
    (outlet.auditResult = "Denied"), (outlet.viewAuditStatus = true);
  }

  if (outlet.ID == NewOutletDefaultID) outlet.IDalias = "";
  else outlet.IDalias = "(" + outlet.ID.toString() + ")";
      ...
      function (dbres) {
        ...
        if (rowLen) {
          ...
          for (i = 0; i < rowLen; i++) {
            ...
            outlet.Distance = calcDistance(saleLoc, {
              Lat: outlet.Latitude,
          ...
          queryOutlets.sort(function (a, b) {
            return a.Distance - b.Distance;
          });

          for (i = 0; i < queryOutlets.length && i < count; i++) {
            var isMatched = true;
            if (config.calc_distance_algorithm == "circle") isMatched = queryOutlets[i].Distance <= meter;
...
var OUTLET = {
  dialog: {
    ...
    outlet: function () {
      return __outletDialogContext.currentOutlet;
    },
    ...
    selectAllUnsyncedOutlets(
      ...
      function (dbres) {
        ...
        if (dbres.rows.length == 0) {
          ...
          OUTLET.submitUnsyncOutlets(nghttp, syncOutletQueue, callback, function (errMessage) {
            callback(errMessage);
          });
        }
      },
  ...
  queryOutletsOnline: function (nghttp, isbackground, view, onSuccess, onError) {
    // type: near-by, new, update...
    try {
      var distance = 200;
      var itemCount = 20;
        ...
        function (response) {
          ...
          try {
            if (data.Status == -1) {
              ...
              foundOutlets.sort(function (a, b) {
                return a.Distance - b.Distance;
              });
              if (!isbackground) {
              ...
              for (var i = 0; i < foundOutlets.length; i++) {
                ...
                if (outlet.PStatus == null || outlet.PStatus == undefined) outlet.PStatus = 0;
                initializeOutlet(outlet);
              }
      ...
      selectOutletsDB(
        ...
        function (dbres) {
          ...
          if (rowLen) {
            ...
            tempOutletList.sort(function (a, b) {
              return a.Distance - b.Distance;
            });

            for (i = 0; i < tempOutletList.length && i < count; i++) {
              var isMatched = true;
              if (config.calc_distance_algorithm == "circle") isMatched = tempOutletList[i].Distance <= meter;
  ...
  queryOutlets: function (nghttp, isbackground, view, callback) {
    ...
    if (!networkReady()) {
      OUTLET.queryOutletsOffline(view, function (isSuccess, outlets) {
        if (!isbackground) hideDlg();

        if (!isSuccess && !isbackground) {
          showError(R.query_outlet_error);
          callback(false, null);
        } else callback(true, outlets);
      });
    } else {
      OUTLET.queryOutletsOnline(
        nghttp,
        isbackground,
        view,
  ...
  searchOutlets: function (nghttp, outletID, outletName, callback) {
    ...
    if (!networkReady()) {
      OUTLET.searchOutletsOffline(outletID, outletName, function (isSuccess, outlets) {
        //hideDlg();
        if (!isSuccess) {
          showError(R.query_outlet_error);
          callback(false, null);
  ...
  searchOutletsOnline: function (nghttp, outletID, outletName, onSuccess, onError) {
    try {
      ...
      cordova.plugin.http.sendRequest(
        ...
        function (response) {
          onError("Cannot search Outlet" + " (#12012)");
        }
      );
    } catch (err) {
      console.error(err);
