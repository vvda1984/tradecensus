...
function homeController($scope, $http, $mdDialog, $mdMedia, $timeout) {
  ...
  $scope.logoutFromHome = function () {
    selectAllUnsyncedOutlets(
      config.tbl_outlet,
      function (dbres) {
        if (dbres.rows.length > 0) {
          showConfirm(
            "Logout",
            "Your data is still not synced to server. Are you sure you want to logout?",
            function () {
              $scope.changeView("login");
            },
            function () {}
  ...
  $scope.expandOutletPanel = function () {
    ...
    if ($scope.showNaviBar) $("#slider-left-content").css("margin-bottom", "48px");
    else $("#slider-left-content").css("margin-bottom", "4px");
  ...
  $scope.showHideRightPanel = function () {
    if (righPanelStatus == 0) {
      if ($scope.journal.viewPanel) $scope.journal.hidePanel();
  ...
  function performPostOutlet(outlet) {
    ...
    showConfirm(
      ...
      function () {
        ...
        OUTLET.saveOutlet($http, clonedOutlet, OUTLET_EDIT, 0, function () {
          $scope.refresh();
        });
  ...
  $scope.revertOutlet = function (outlet) {
    ...
    showConfirm(
      ...
      function () {
        ...
        saveOutlet(clonedOutlet, function (synced) {
          log("Save outlet to local db");
          changeOutletStatusDB(
            $scope.config.tbl_outlet,
            clonedOutlet,
            clonedOutlet.AuditStatus,
            synced ? 1 : 0,
            function () {
              hideDlg();
              $scope.refresh();
  ...
  $scope.deleteOfflineOutlets = function () {
    ...
    showConfirm(
      R.confirm,
      R.delete_offline_outlets_of + p.name + "?",
      function () {
        selectUnsyncedOutletsOfProvince(
          $scope.config.tbl_outlet,
          p.id,
          function (dbres) {
            if (dbres.rows.length > 0) {
            ...
            deleleDownloadProvinceDB(
              $scope.config.tbl_outlet,
              $scope.config.tbl_downloadProvince,
              p.id,
              function () {
                //hideDlg();
                //p.download = 0;
  ...
  $scope.downloadOutlets = function () {
    ...
    for (var i = 0; i < $scope.provinces.length; i++) {
      if ($scope.provinces[i].download && p.id !== $scope.provinces[i].id) downloadedCount++;
    ...
    try {
      ...
      selectUnsyncedOutletsDB(
        ...
        function (dbres) {
          ...
          showConfirm(
            R.download_outlets,
            R.download_outlets_confim + selectProvince.name + "?",
            function () {
              setTimeout(function () {
                downloadSession++;
                try {
                  showLoading(R.downloading_outlet, R.please_wait, R.cancel_download_confirm, function () {
                  ...
                  cordova.plugin.http.sendRequest(
                    ...
                    function (response) {
                      ...
                      try {
                        var outletCount = JSON.parse(response.data);
                        if (outletCount > 0) {
                          deleleOutletsDB(
                            config.tbl_outlet,
                            selectProvince.id,
                                ...
                                function () {
                                  downloadDistricts(addressModel.provinceRaw.referenceGeoID, function () {
                                    ...
                                    if (sessionID == downloadSession) {
                                      changeDownloadProvinceStatusDB(
      ...
      dialogUtils.showClosableDlg("Load sub border data", R.please_wait, function (hideDlgFunc, isCancelledFunc) {
        ...
        cordova.plugin.http.sendRequest(
          ...
          function (response) {
            ...
            tcutils.logging.error(response.error);
            callback();
          }
        );
      });
    ...
    tryOpenDialog(function () {
      ...
      OUTLET.dialog.open(
        ...
        function (answer, outlet) {
          ...
          OUTLET.saveOutlet($http, outlet, OUTLET_NEW, 0, function () {
            $scope.refresh();
          });
        }
      );
      ...
      cordova.plugin.http.sendRequest(
        ...
        function (response) {
          try {
            ...
            var s1 = response.data.replaceAll('\\"', '"');
            var s1 = s1.replaceAll('\\\\"', " ");
            var s2 = s1.substring(1, s1.length - 1);
            var items = [];
            try {
              items = JSON.parse(s2);
            } catch (err) {
              console.info("Download oulets", { s1, s2 });
              onError(err);
            }
            log("Download " + items.length + "outlet(s):" + JSON.stringify(items[0]));

            if (items.length > 0) {
              showLoading(downloadMessage, R.downloading_outlet_save_outlets);
              addDownloadOutletsDB(
                config.tbl_outlet,
                items,
                function () {
                  if (cancelLoadingDlg || sessionID != downloadSession) {
                    ...
                    if (page + 1 < maxPage) downloadProvinceOutletsZip(provinceID, sessionID, page + 1, maxPage, onSuccess, onError, onCancel);
                    else onSuccess();
                  }
                },
                function (tx11, dberr) {
  ...
  function loadOutlets(outlets) {
    ...
    if (isMapReady && networkReady()) {
      loadMarkers(curOutletView == 1, outlets, function () {
        ...
        OUTLET.dialog.open(
          ...
          function (answer, outlet) {
            ...
            if (!isSendNewOutletRequest) {
              if (!outlet.isRevert) {
                if (outlet.IsTracked) outlet.Tracking = 1;
                else outlet.Tracking = 0;
            ...
            if (outlet.isDeleted) {
              OUTLET.saveOutlet($http, orgOutlet, OUTLET_DELETE, curOutletView, function () {
                $scope.refresh();
              });
            } else {
              if (curOutletView != 1) {
              ...
              OUTLET.saveOutlet($http, orgOutlet, OUTLET_EDIT, curOutletView, function () {
                $scope.refresh();
              });
            }
          }
  ...
  function submitUnsyncedOutlets(unsyncedOutlets, onSuccess, onError) {
    if (networkReady()) {
      ...
      cordova.plugin.http.sendRequest(
        ...
        function (response) {
          console.info("Save Unsynced Outlet completed...", response);
          const data = JSON.parse(response.data);
          log(response);
          if (data.Status == -1) {
            // error
  ...
  function handleMapLoaded() {
    showCurPositionDlg(
      ...
      function (lat, lng) {
        ...
        if (!__isOutletDlgOpen) {
          if (tcutils.tcapp.lastRefreshOutletsTS == null) tcutils.tcapp.lastRefreshOutletsTS = new Date();
  ...
  $scope.changePassword = function (isCancel) {
    if (isCancel) {
      ...
      if (!$scope.forceChangePassword && isEmpty($scope.curPass)) {
        showErrorAdv(R.current_pass_is_empty, function () {
          $("#cur-pass").focus();
        });
        return;
      }
      ...
      if (isEmpty($scope.newPass)) {
        showErrorAdv(R.new_pass_is_empty, function () {
          $("#new-pass").focus();
        });
        return;
      }
      ...
      if ($scope.newPass != $scope.confirmPass) {
        showErrorAdv(R.confirm_pass_doesnot_match, function () {
          $("#confirm-pass").focus();
        });
        return;
      }
      ...
      try {
        ...
        cordova.plugin.http.sendRequest(
          ...
          function (response) {
            log("HTTP error...");
            var msg = "" + response.status + ":" + response.error; // err.statusText == "" ? $scope.R.text_ConnectionTimeout : err.statusText;
            showError(msg);
          }
        );
      } catch (ex) {
  ...
  $scope.drawBorder = function () {
    ...
    if ($scope.selectedBorder.GeoData != null && $scope.selectedBorder.GeoData != "") drawMapBorder($scope.selectedBorder.GeoData);
    else {
  ...
  $scope.journal = {
    ...
    showPanel: function () {
      if (righPanelStatus === 1) $scope.cfg.hidePanel();
    ...
    showHideJournalPanel: function () {
      if (righPanelStatus === 1) $scope.showHideRightPanel();
  ...
  function submitJournal(journal, callback) {
    if (tcutils.networks.isReady()) {
      ...
      cordova.plugin.http.sendRequest(
        ...
        function (response) {
          tcutils.writeLog("Submit error: " + response.error);
          tcutils.writeLog(response);
          callback(false);
        }
      );
    ...
    if (tcutils.networks.isReady()) {
      ...
      cordova.plugin.http.sendRequest(
        ...
        function (response) {
          ...
          tcutils.writeLog(response.error);
          callback(false, null);
        }
      );
    } else {
