  ...
  if (!networkReady()) {
    ...
    if (loadedMapAPI) {
      ...
      $.getScript(url, function (data, textStatus, jqxhr) {
        if (hideMaskedDlg) hideMaskedDlg();
...
function createMaker(outlet, position, i, isNew) {
  ...
  if (!outlet.IsOpened || !outlet.IsTracked) zindexval = google.maps.Marker.MAX_ZINDEX + 2;
  ...
  marker.addListener("click", function () {
    ...
    if (curInfoWin != null) curInfoWin.close();
    infoWindow.open(map, marker);
...
function getMarkerIcon(outlet) {
  if (outlet != null) {
    if (
      ...
      var imgName = null;
      if (outlet.InputByRole == 0) {
        // Sale
        imgName = config.map_tc_salesman_outlet;
      } else if (outlet.InputByRole == 1) {
        // Sale auditor
        imgName = config.map_tc_auditor_outlet;
      } else if (outlet.InputByRole == 2) {
        // Agency
      ...
      if (outlet.AmendByRole == 2 || outlet.AmendByRole == 3) {
        if (outlet.AuditStatus == StatusAuditDeny || outlet.AuditStatus == StatusExitingDeny) {
          if (isEmpty(config.map_tc_agency_existing_outlet_denied)) return "assets/img/pin-dis-audit-error.png";
          else return "data:image/png;base64," + config.map_tc_agency_existing_outlet_denied;
        }
        if (outlet.AuditStatus == StatusAuditAccept || outlet.AuditStatus == StatusExitingAccept) {
          if (isEmpty(config.map_tc_agency_existing_outlet_approved)) return "assets/img/pin-dis-audit-right.png";
          else return "data:image/png;base64," + config.map_tc_agency_existing_outlet_approved;
        }
        ...
        switch (outlet.OutletSource) {
          case 0: //SR
            if (outlet.AuditStatus == StatusAuditDeny || outlet.AuditStatus == StatusExitingDeny) {
              if (isEmpty(config.map_sr_outlet_audit_denied)) return "assets/img/pin-sr-audit-wrong.png";
              else return "data:image/png;base64," + config.map_sr_outlet_audit_denied;
            }
          ...
          case 1: // DIS
            if (outlet.AuditStatus == StatusAuditDeny || outlet.AuditStatus == StatusExitingDeny) {
              if (isEmpty(config.map_dis_outlet_audit_denied)) return "assets/img/pin-dis-audit-error.png";
              else return "data:image/png;base64," + config.map_dis_outlet_audit_denied;
            }

            if (outlet.AuditStatus == StatusAuditAccept || outlet.AuditStatus == StatusExitingAccept) {
              if (isEmpty(config.map_dis_outlet_audit_approved)) return "assets/img/pin-dis-audit-right.png";
              else return "data:image/png;base64," + config.map_dis_outlet_audit_approved;
            }
            ...
            //}
            if (!outlet.IsOpened) {
              if (isEmpty(config.map_dis_outlet_closed)) return "assets/img/pin-dis-close.png";
              else return "data:image/png;base64," + config.map_dis_outlet_closed;
            }
...
function markerClick(i) {
  if (markerClicked != null) markerClicked(i);
}
  ...
  if (gpsWatchID == -1) {
    gpsWatchID = navigator.geolocation.watchPosition(
      function (position) {
        ...
        if (__locationChangedCallback) __locationChangedCallback(foundLat, foundLng, foundAcc);
      },
      function (error) {
...
function calcDistance(saleLoc, outletLoc) {
  ...
  var a =
    ...
    Math.cos(calculateRad(saleLoc.Lat)) * Math.cos(calculateRad(outletLoc.Lat)) * Math.sin(dLong / 2) * Math.sin(dLong / 2);
...
function isGPSAvailable(callback) {
  if (_WEB) {
    ...
    //navigator.geolocation.getCurrentPosition(callback(true), callback(false));
