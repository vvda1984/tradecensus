...
function dlgClose() {
  log("Close dialog.");
  hideDlg();
  if (dlgCloseCallback) dlgCloseCallback();
}
...
function dlgConfirmClose(i) {
  ...
  if (i == 1) {
    ...
    if (dlgDenyCallback != null) dlgDenyCallback();
  }
}
  ...
  if (config.enable_devmode) {
    ...
    navigator.notification.confirm(
      ...
      function (i) {
        if (i == 2) {
          hideLoadingDlg();
          if (loadingCloseCallback != null) loadingCloseCallback();
        }
...
function showLoading(title, message, closeMsg, callback) {
  ...
  if (isLoadingDlgOpened) {
    ...
    cover =
      ...
      '<div class="dialog">' +
      '<div class="content">' +
      '<div id="loading-dlg-title" class="title">' +
      title +
      "</div><br>" +
      '<div id="loading-dlg-message">' +
      message +
      "</div>" +
      "</div>" +
...
function __closeProcessingDlg() {
  if (__processingSettings.cancelConfirmMessage !== null && __processingSettings.cancelConfirmMessage.length > 0) {
    if (config.enable_devmode) {
      ...
      if (__processingSettings.cancelCallback) __processingSettings.cancelCallback();
    } else {
      navigator.notification.confirm(
...
function __closeGetLocationErrorDlg(retry) {
  if (retry) {
    if (__getLocationErrorSettings.retryCallback) __getLocationErrorSettings.retryCallback();
  } else {
    if (__getLocationErrorSettings.closeCallback) __getLocationErrorSettings.closeCallback();
  }
  try {
    $("#dlg-panel").remove();
  } catch (e) {
...
var dialogUtils = {
  ...
  showClosableDlg: function (title, message, action) {
    ...
    var __hideLoading = function () {
      dialogUtils.hideProcessing();
    };
    var __isCancelled = function () {
      return isCancelled;
    };
