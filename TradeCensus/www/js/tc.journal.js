var journals = {
  ...
  _newJournalPolyline: function (line) {
    if (journals._polyline) journals._expiredLines.push(journals._polyline);
  ...
  trackJournal: function (newLat, newLng, acc) {
    ...
    if (journals.validateNewLocation(newLat, newLng, acc))
      journals._track(function () {
        tcutils.logging.debug("Tracked current location to journal");
      });
  },
        ...
        function (tx, dbres) {
          ...
          if (dbres.rows.length > 0) {
            ...
            for (var i = 0; i < dbres.rows.length; i++) {
              ...
              if (item.journalDate != null && item.journalDate != undefined) unsyncedItems[index++] = item;
            }
              ...
              if (isSucceeded) {
                for (var i = 0; i < items; i++) {
                  ...
                  var updateSql =
                    ...
                    ", " +
                    "status = " +
                    journals.statusEnum.synced.toString() +
                    " " +
                    'WHERE journalId = "' +
                    item.journalId +
                    '"';
    ...
    if (journals._query) {
      ...
      journals._query(tcutils.formatDate(dateFrom), tcutils.formatDate(dateTo), function (isSucceeded, items) {
        if (isSucceeded) {
          ...
          } else tcutils.messageBox.info("No journals for selected date(s)!");
        }
      });
    }
  },
