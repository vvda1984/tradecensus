  ...
  db.transaction(
    function (tx) {
      items.sort(function (i1, i2) {
        if (i1.KPIType < i2.KPIType) return -1;
        if (i1.KPIType > i2.KPIType) return 1;
        return 0;
      });
...
function ensureUserOutletDBExist(isReset, outletSyncTbl, outletTbl, provinceDownloadTbl, journalTbl, supplierTbl, callback) {
  db.transaction(function (tx) {
    if (isReset) {
      ...
      tx.executeSql(
        "Delete from " + outletTbl,
        [],
        function (tx1) {},
        function (tx1, dberr) {}
      );
      tx.executeSql(
        "Delete from " + provinceDownloadTbl,
        [],
        function (tx1) {},
        function (tx1, dberr) {}
      );
      tx.executeSql(
        "Delete from " + outletSyncTbl,
        [],
        function (tx1) {},
        function (tx1, dberr) {}
      );
...
function addDownloadOutletsDB(outletTbl, outlets, onSuccess, onError) {
  ...
  db.transaction(function (tx) {
    ...
    if (err == null) {
      tx.executeSql(
        "select * from " + outletTbl + ' where Name = "Aaaa"',
        [],
        function (tx1, dbres) {
...
function _updateOutlet(tx, outletTbl, outlet, state, synced, updateImage) {
  ...
  if (updateImage) {
    ...
    } else sql = sql.concat("StringImage1='',");
    ...
    } else sql = sql.concat("StringImage2='',");
    ...
    } else sql = sql.concat("StringImage3='', ");
    ...
    } else sql = sql.concat("StringImage4='',");
    ...
    } else sql = sql.concat("StringImage5='',");
    ...
    } else sql = sql.concat("StringImage6='', ");
  }
  ...
  db.transaction(function (tx) {
    ...
    try {
      ...
      for (var i in syncOutlets) {
        ...
        var sql = "UPDATE " + outletTbl + " SET ID = " + outlet.ID.toString() + "," + " PSynced = " + s + ' WHERE PRowID = "' + outlet.RowID + '"';
        tx.executeSql(
          sql,
          [],
          function () {},
  ...
  db.transaction(
    function (tx) {
      ...
      tx.executeSql(
        ...
        [],
        function (tx1, dbres) {
          if (dbres.rows.length > 0) callback(dbres.rows.item(0));
          else callback(null);
        },
        function (tx1, err) {
          callback(null);
...
function selectNearByOutlets(outletTbl, latMin, latMax, lngMin, lngMax, view, synced, onSuccess, onError) {
  db.transaction(function (tx) {
    ...
    if (view == 0) {
      ...
      sql = sql.concat(
        "AuditStatus in (",
        StatusExitingAccept.toString(),
        ", ",
        StatusExitingDeny.toString(),
        ", ",
        StatusAuditAccept.toString(),
        ", ",
        StatusAuditDeny.toString(),
...
function insertImageUploadingInfo(tx, userID, outletID, index, imagePath, now) {
  ...
  var sql =
    "INSERT INTO [outletImage1] ([ID], [OutletID], [ImageIndex] , [ImagePath], [Uploaded], [CreatedDate], [CreatedBy] ) VALUES (" +
    '"' +
    id +
    '", ' +
    outletID.toString() +
  ...
  tx.executeSql(
    ...
    [],
    function () {
      log("store upload info of image1");
    },
    function (dberr) {
      log(dberr.message);
    }
...
function selectUnsyncedOutletImage(userID, outletID, onSuccess, onError) {
  db.transaction(function (tx) {
    ...
    var sql =
      "SELECT * FROM [outletImage1] WHERE" + " OutletID = " + outletID.toString() + " AND CreatedBy = " + userID.toString() + " AND Uploaded = 0";
    logSqlCommand(sql);
    tx.executeSql(sql, [], onSuccess, onError);
  }, onError);
}
...
function selectUserOutletsDB(outletTbl, start, end, onSuccess, onError) {
  db.transaction(function (tx) {
    var sql =
      ...
      "] WHERE AmendBy=" +
      userID.toString() +
      " AND AuditStatus > 0 AND PLastModTS >= " +
      start.toString() +
      " AND PLastModTS <= " +
      end.toString() +
      " ORDER BY PLastModTS";
