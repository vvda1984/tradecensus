...
function editOutletController($scope, $mdDialog, $timeout) {
  ...
  $scope.disableOutletComment = false;
  ...
  if ($scope.outlet.AuditStatus == 0) {
    ...
    if (user.role == 1) {
      ...
      isSameSystem = true;
    } else if (user.role == 0 && ($scope.outlet.AmendByRole == 0 || $scope.outlet.AmendByRole == 1)) isSameSystem = true;
    else if ((user.role == 2 || user.role == 3) && ($scope.outlet.AmendByRole == 2 || $scope.outlet.AmendByRole == 3)) isSameSystem = true;
  }

  if (!user.hasAuditRole) {
    isSameSystem = true;
  } else {
    $scope.disableOutletComment = true;
  }
    ...
    if ($scope.outlet.AuditStatus == StatusAuditAccept || $scope.outlet.AuditStatus == StatusAuditDeny || $scope.outlet.AuditStatus == StatusDone) {
      ...
      if (user.hasAuditRole && $scope.outlet.PersonID != userID) {
        ...
        if ($scope.outlet.AuditStatus == StatusInitial) {
          ...
          if ($scope.outlet.IsOpened) $scope.disableTracking = $scope.outlet.Tracking == 1;
          else $scope.disableTracking = false;
        } else {
          setViewOnly();
        }
      } else {
        ...
        if ($scope.outlet.AuditStatus == StatusInitial) {
          ...
          setViewOnly();
          $scope.canRevise = $scope.outlet.canRevise;
  ...
  $scope.approve = function () {
    ...
    checkDistance(false, function () {
      log("Change open/close status: " + $scope.outlet.IsOpened);
      setOpenCloseValue();
      if ($scope.outlet.AuditStatus == StatusExitingPost) $scope.outlet.AuditStatus = StatusExitingAccept;
      else $scope.outlet.AuditStatus = StatusAuditAccept;
  ...
  $scope.deny = function () {
    ...
    checkDistance(false, function () {
      ...
      if ($scope.outlet.AuditStatus == StatusExitingPost) $scope.outlet.AuditStatus = StatusExitingDeny;
      else $scope.outlet.AuditStatus = StatusAuditDeny;
  ...
  function setOpenCloseValue() {
    ...
    if ($scope.outlet.IsOpened) {
      ...
      if ($scope.outlet.CloseDate == "") $scope.outlet.CloseDate = currentDate();
    }
  }
